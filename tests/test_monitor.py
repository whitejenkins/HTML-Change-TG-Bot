...
class MonitorChangeDetectionTest(unittest.TestCase):
    ...
    def test_check_once_reports_progress_for_each_page(self):
        self.monitor.update_config(send_screenshot_on_change=False)
        state = self.monitor.get_state()
        state["pages"] = {
        ...
        self.monitor.save_state_unlocked(state)
        self.monitor.fetch_html = lambda url, timeout: url
        self.monitor.normalize_html = lambda html, ignore_patterns: (html, [])

        progress = []
        self.monitor.check_once(progress_callback=lambda index, done, status: progress.append((index, done, status)))

        self.assertEqual(
            progress,
            [("1", 0, "checking"), ("1", 1, "done"), ("2", 1, "checking"), ("2", 2, "done")],
        )
        self.assertIn("2/2", self.monitor.format_check_progress(["1", "2"], None, 2, "finished"))
